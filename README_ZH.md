...
重新安排它们，再把各自的新起始位置写回主头和 FlashMe 头。组件未指定 `--offset` 时，
工具按确定性的 FlashMe 顺序排列，再以转换后的大小和头中对齐要求决定其地址。每个
组件及次级头本身都可通过局部的 `--offset` 固定到指定物理地址。
...
对应默认位置。未显式指定地址的组件会一起自动重排；显式与自动放置可以混用，并会
按同一张布局表检查碰撞。
...
以下参数会在 P1/P2 加密之前修改所提供的主头。创建 FlashMe 时，最终主头会修复
FH 中的所有公共字段；但 FP1/FP2 的源地址与 RAM 地址描述符（`0x0C`--`0x13`）和
FH 自己的类型标记（`0x17C`）保留 `-fh` 文件中的值，随后再按最终布局重写 FP1/FP2
的源地址。最终 CRC 字段及原固件标记会从 H 同步到 FH，与官方 FlashMe V1--V8 模板
一致：
...
配置校验区，也会重新计算其校验和。普通固件还会重新计算三组主组件 CRC16；FlashMe
创建会重算 H 的 P1--P5 CRC16，再将最终 CRC 字段同步至 FH，同时保留 FH 自己的
FP1/FP2 描述符和类型标记，与官方 FlashMe V1--V8 布局一致。
